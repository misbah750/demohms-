...
const MOCK_PATIENT = {
  ...
  age: 28, gender: "female", medical_history: "No major conditions. Regular checkups.", allergies: "Penicillin",
};
...
export default function EditPatientPage() {
  ...
  const [form, setForm] = useState<PatientFormData>({
    name: "", phone: "", email: "", age: "", gender: "female", medical_history: "", allergies: ""
  });
  const [errors, setErrors] = useState<Partial<PatientFormData>>({});

  const { data: patient, isSuccess } = useQuery({
    queryKey: ["patient", id],
  ...
  function field(key: keyof PatientFormData, label: string, opts?: { type?: string; placeholder?: string; as?: "textarea" }) {
    ...
    return (
      <div className="space-y-1.5">
        ...
        {hasError && <p className="text-xs text-red-400">{errors[key]}</p>}
      </div>
    );
  }
    ...
    <div className="max-w-2xl space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">Modify patient record information</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white border border-slate-200 rounded-2xl p-6 space-y-6 shadow-sm">
        ...
        <div className="flex items-center gap-3 pb-3 border-b border-slate-100">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">Edit core patient info</p>
          </div>
        </div>
        ...
        <div className="pt-4 border-t border-slate-100 space-y-4">
          ...
          {field("medical_history", "Medical History", { as: "textarea" })}
